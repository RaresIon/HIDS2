...
def init_probes():
    ...
    print("attached all probes")
    print("will return bpf ",bpf)
    return (bpf)

    
def gather_data(mydb,bpf):
    ...
    def store_event(cpu, data, size):
        ...
        if pid_dict.get(event.pid,0) < 5000:
        ...
        if len(commit_list) == 1000 :
            try:
                ...
                mydb.commit()
                commit_list = []
                nr_commits = nr_commits + 1
            except:
                print("commit failed")

        if nr_commits == 2000:        
            sys.exit()    
        print(len(pid_dict)," processes found") 
        print(len(commit_list)," entries ")
        print(nr_commits," commits made ")
